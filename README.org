...
moving point into a source row reveals that row for editing.  Resizing or
editing rebuilds the view.  This integration currently uses an internal
~markdown-table-wrap~ rendering API while its public display API is finalized.
