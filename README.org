...
- ~markdown-ts-appear-label-caps~ optionally replaces label brackets with left
  and right strings such as ~("" . "")~.  Label text uses inverse video while
  the caps use its background color as their foreground, forming one shape.
