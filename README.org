...
| ~markdown-ts-appear-table-style~         | ~raw~   | ~raw~, ~unicode~, or width-aware ~wrapped~ tables.       |
...
With ~markdown-ts-inline-images~ enabled, local images on their own lines are
displayed as cached image slices in graphical Emacs.  Use relative or absolute
paths in ~![description](path)~ links.  Images that fit the preview are shown in
full; larger images use a preview no taller than about half the window, with a
visible row range.  Visual-line motions (~j/k~ when configured for visual lines)
move through the slices, scrolling the preview as needed.  Clicking a slice
selects it.  Source tracking reveals the original image link for editing.
Visible slices have separate display anchors inside the existing link, so they
can scroll through the window edge individually without inserting buffer text.
Short link destinations also limit the preview height; all slices remain
accessible with visual-line movement.

Set buffer-local ~line-spacing~ to ~0~ for seamless slices; Emacs otherwise
adds the configured prose spacing between them.  ~markdown-ts-image-max-width~
controls image width.  Window resizing reuses cached file data.  Toggle
~markdown-ts-toggle-inline-images~ off and on to refresh changed image files.
Remote images and images embedded within prose retain native handling rather
than the sliced block-image view.  Wrapped table cells keep image labels.
...
the source position of each visible character, including across wrapped cells.
Entering insert state edits that same source character.  Hidden markup uses
...
Visual-line motions retain their display column across wrapped cells; logical
...
- Math rendering uses ~mathjax-display~.  There are no separate cache, timeout,
  ...
  inline formulas stay in the surrounding text.  Resizing uses the same image.
  Ordinary cursor movement updates visibility without rescanning the document.
...
~test/markdown-ts-appear-image-test.el~ also includes real GUI image-geometry
checks.  To run them in a disposable graphical Emacs (not ~--batch~):
