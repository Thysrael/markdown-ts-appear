...
displayed whole in graphical Emacs.  Use relative or absolute paths in
~![description](path)~ links.  Large images are scaled down proportionally to
fit the window width and usable height, reserving room for the caption and
scroll margins.  Smaller images are not enlarged.  Path length does not limit
the display height, and there is no cropped image panel or slice navigation.

Visual-line motions (~j/k~ when configured for visual lines) retain native
cursor movement: an image is one display object.  The document scrolls by
pixels at image edges, with a short transition when crossing the whole image.
Source tracking reveals the original image link for editing; the Markdown
text itself is never changed by rendering or navigation.

~markdown-ts-image-max-width~ sets an additional width limit.  Window resizing
reuses cached file data and adjusts the whole image's scale.  Toggle
...
than the whole block-image view.  Wrapped table cells keep image labels.
