...
paths in ~![description](path)~ links.  Small images are shown in full; tall
images use a preview no taller than about half the window, with a visible row
range.  Visual-line motions (~j/k~ when configured for visual lines)
