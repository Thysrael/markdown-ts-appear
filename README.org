...
Icon and quote marker values are plain strings and do not require an icon
package.  A cons cell selects its first string when every character is
displayable and its fallback otherwise.  For example:
...
- ~markdown-ts-appear-link-icon~, ~markdown-ts-appear-image-icon~, and
  ...
  nil.
- ~markdown-ts-appear-code-fence-style~ is either ~raw~ or ~connected~ and
  defaults to ~raw~.  The connected style displays ~╭─ language~, prefixes
  code lines with ~│~, and displays ~╰─~ for the closing fence.
- ~markdown-ts-appear-label-caps~ optionally surrounds label text with left and
  right strings such as ~("" . "")~.  Label text uses inverse video while the
  caps use its background color as their foreground, forming one shape.
- ~markdown-ts-appear-render-callouts~ recognizes callout markers such as
  ~[!WARNING]-~ at the start of block quotes.  Callout and code-language labels
  use inverse video, but callout folding is not implemented.
- ~markdown-ts-appear-block-quote-marker~ controls the marker displayed on each
  quoted line and defaults to nil.
